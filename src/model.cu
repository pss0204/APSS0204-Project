...
void generate_images(float *input, float *output, size_t n_img) {
	...
	for (size_t n = 0; n < n_img; n++) {
		...
		//printf("%f",*input);
		CHECK_CUDA(cudaMemcpy(z->d_buf, input + n * LATENT_DIM, LATENT_DIM * sizeof(float),cudaMemcpyHostToDevice));//pss
		// cudaMemcpy
		...
		CHECK_CUDA(cudaGetLastError());
		CHECK_CUDA(cudaMemcpy(output + n * 3 * 128 * 128, conv_a->d_buf, 3 * 128 * 128 * sizeof(float),cudaMemcpyDeviceToHost));//pss
		// cudaMemcpy
	}
}