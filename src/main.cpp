...
static char input_fname[100] = "./data/input_fp32.bin";
static char param_fname[100] = "/opt/apss24/project/param_fp32.bin";
...
void print_help() {
  ...
  fprintf(stdout, 
          "  -i: Input binary path (default: ./data/input_fp32.bin)\n");
  fprintf(stdout,
          "  -p: Model parameter path (default: /opt/apss24/project/param_fp32.bin)\n");
  fprintf(stdout, 
...
void *read_binary(const char *fname, size_t *size) {
  ...
  if (size != NULL) *size = (size_t)(size_ / 4);  // 4 bytes per float
...
int main(int argc, char **argv) {
  ...
  size_t input_size;
  input = (float *) read_binary(input_fname, &input_size);

  /* Allocate output (size: [(num_images) * (3 * 128 * 128)]) */
  output = (float *) malloc(num_images * num_elems_per_image * sizeof(float));
  ...
  write_binary((float *) output, output_fname, num_images * num_elems_per_image);
  fprintf(stdout, "Done!\n");
  ...
  if (run_validation) {
    ...
    if (ret == -1) {
      ...
      fprintf(stdout, "FAILED!\nFirst mismatch "
                      ...
                      (int) (ret / num_elems_per_image), 
                      ret % num_elems_per_image,
                      ret, output[ret], ret,
                      answer[ret]);
    }
  }
